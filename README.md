...
---
## TODOS:
- improve login (ssh with keys and reasonable storage for them)
- ship everything (even the kiosk mode browser as a single binary)
- i could even use something like egui which i wanted to try or iced
...
Why We Abandoned Browser-Based Kiosk on Raspberry Pi Zero
Root Cause

ARMv6 architecture incompatibility with modern web rendering engines on Raspberry Pi Zero.
Issues Encountered

    Surf (WebKitGTK): Floating point exception (exit code 136) - WebKitGTK not compatible with ARMv6
    NetSurf-GTK: Launched but displayed black screen despite window creation
    Links2: Works but has zero CSS support - renders unstyled tables only
    Dillo: Couldn't initialize display properly
    Full KMS driver: Display pipeline issue - framebuffer renders correctly but HDMI output stays black

Working Configuration (Fallback)

    Display Driver: vc4-fkms-v3d (fake KMS) in /boot/firmware/config.txt
    Display Manager: LightDM with autologin
    Browser: Links2 (functional but no CSS)
    X Server: Working with fkms driver

Why This Matters for Future Hardware

On non-ARMv6 hardware (Pi 3+, Pi 4, x86), modern browsers work fine:

    Chromium in kiosk mode
    Firefox
    Surf with proper WebKitGTK support
